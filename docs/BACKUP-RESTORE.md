...
### Option 1: Google Drive (using rclone) ⭐ RECOMMENDED

Google Drive provides 15GB free storage, perfect for automated off-site backups.
...
- `n` - New remote
- Name: `gdrive`
- Storage type: `Google Drive` (usually option 15)
...
You'll get a URL - **IMPORTANT**:
1. Copy the URL from your SSH terminal
...
rclone mkdir gdrive:Backups/retirement-app
```
...
rclone copy /tmp/test.txt gdrive:Backups/retirement-app/
rclone ls gdrive:Backups/retirement-app/

# Clean up test
rclone delete gdrive:Backups/retirement-app/test.txt
rm /tmp/test.txt
```

5. **Backup script is already configured** - Google Drive upload is now enabled automatically!
...
# Remove old backup
rclone delete gdrive:Backups/retirement-app/<old-backup>.tar.gz

# Sync folder (two-way)
rclone sync ~/backups/ gdrive:Backups/retirement-app/
```

**Storage Management**:
- Backups auto-upload after each local backup
- Keeps last 14 backups on Google Drive (older auto-deleted)
- Each backup typically 15-65 MB
- 15GB free = ~230-1000 backups
- Monitor: `rclone about gdrive:`
